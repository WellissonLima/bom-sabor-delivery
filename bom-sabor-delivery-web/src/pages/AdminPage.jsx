      ...
      <div className="max-w-4xl mx-auto">
        <div className="flex justify-between items-center mb-8">
          ...
          <button
            ...
            className="ml-auto mb-6 flex items-center gap-2 text-gray-500 hover:text-red-500 transition-colors"
          >
        ...
        >
          ...
          {newProduct.category === "pizza" ? (
            <div className="bg-red-50 p-4 rounded-xl space-y-3">
              <p className="font-bold text-pizza-red flex items-center gap-2">
                <Pizza size={18} /> Preços Fixos (R$)
              </p>
              <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                {["P", "M", "G", "F"].map((t) => (
                  <div key={t}>
                    <label className="text-xs font-bold text-gray-500 text-center block">
                      Tamanho {t}
                    </label>
                    <input
                      type="number"
          ...
          <button
            type="submit"
            className={`w-full font-black py-4 rounded-xl transition-all ${editingId ? "bg-blue-600" : "bg-pizza-red"} text-white`}
          >
        ...
        <div className="grid gap-3">
          {products.map((p) => (
            <div
              key={p._id}
              className="bg-white p-4 rounded-xl shadow-sm flex justify-between items-center border border-gray-100 hover:border-gray-300 transition-all"
            >
              <div>
                <span className="text-[10px] font-black uppercase bg-gray-100 px-2 py-1 rounded text-gray-500 mr-2">
                  {p.category}
                </span>
